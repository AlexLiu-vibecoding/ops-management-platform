...
| 版本 | 日期 | 更新内容 |
|------|------|----------|
| v3.0 | 2026-04-08 | 修复解密逻辑从数据库读取密钥，支持动态多版本 |
...
| 痛点 | 描述 | RIPER 解法 |
|------|------|-----------|
| **上下文腐烂** | 对话越长，AI 越容易遗忘前文约束 | Spec 作为持久化上下文 |
| **审查瘫痪** | AI 秒生成 500 行，人工审查不过来 | 分阶段审批，控制粒度 |
| **维护断层** | 两周后不敢动 AI 生成的代码 | 决策留痕，Spec 溯源 |
| **代码不信任** | 不知道 AI 为什么这么写 | Plan 阶段人类拍板 |
...
│                                                                             │
│  ┌──────────────┐    ┌──────────────┐    ┌──────────────┐               │
│  │   Pre-       │───▶│   Research   │───▶│   Innovate    │               │
│  │   Research   │    │   (研究)      │    │   (创新)       │               │
│  │   (准备)      │    │              │    │               │               │
│  └──────────────┘    └──────────────┘    └──────────────┘               │
│         │                   │                   │                          │
│         │   理解现有代码    │   分析问题本质    │   提出 2-3 个方案        │
│         │   准备 Context    │   锁定事实       │   对比 Pros/Cons        │
│         │                  │                   │                          │
│         ▼                   ▼                   ▼                          │
│  ┌──────────────┐    ┌──────────────┐    ┌──────────────┐               │
│  │     Plan     │◀───│              │    │   Execute     │               │
...
| 阶段 | 创造力 | 核心任务 | 产出物 |
|------|--------|----------|--------|
| **Pre-Research** | 低 | 理解现有代码，准备 AI 需要的"原材料" | Context Bundle |
| **Research** | 低 | 扫描代码库，锁定事实，消除信息差 | Code Facts |
| **Innovate** | 高 | 给出 2-3 个方案，对比 Pros/Cons | Options + Analysis |
| **Plan** | 低 | 原子级拆解，精确到文件路径和函数签名 | **Spec 文档** |
| **Execute** | 零 | 严格按图施工，AI 是打字员 | 代码 |
| **Review** | 中 | 校验 Spec 与 Code 一致性 | 验证报告 |
...
- Innovate 阶段放开创造力
- Execute 阶段收紧执行力
- 大部分人的问题：讨论时急着干活，干活时自由发挥

⚠️ 门禁规则：
- 未完成 Plan 审批，不得进入 Execute 阶段
- 看不懂 Plan，就不准动手
- Review 必须有依据，不能凭感觉
```
...
Spec 首先是**给人类**的，不是给 AI 的！

| 受益者 | Spec 解决的问题 |
|--------|---------------|
| 开发者自己 | 半年没碰的项目，读 Spec 20 分钟就能重新进入状态 |
| 接手的同事 | 交接 = 读 Spec + 按图施工，接入周期从周级降到天级 |
| TL / 主管 | 打开 Spec 就知道需求做到哪、决策了什么、还有什么风险 |
| 团队质量 | 每次修改都有决策留痕、有 Plan 审批、有 Review 闭环 |
