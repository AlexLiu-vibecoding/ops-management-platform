  ...
  <div class="approvals-page">
    <el-card shadow="never">
      ...
      <el-table :data="approvalList" style="width: 100%" v-loading="loading">
        ...
        <el-table-column prop="database_target" label="目标数据库" min-width="120" show-overflow-tooltip>
          <template #default="{ row }">
            <span v-if="row.database_mode === 'all'">
              <el-tag type="warning" size="small">全部数据库</el-tag>
            </span>
            <span v-else-if="row.database_mode === 'pattern'">
              <el-tag type="info" size="small">{{ row.database_pattern || '通配符匹配' }}</el-tag>
            </span>
            <span v-else-if="row.database_mode === 'auto'">
              <el-tag type="success" size="small">SQL自动解析</el-tag>
            </span>
        ...
        <el-table-column prop="affected_rows_estimate" label="预估影响" width="90" align="right">
          <template #default="{ row }">
