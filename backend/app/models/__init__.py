...
class ApprovalRecord(Base):
    ...
    sql_content = Column(Text, comment="SQL内容（小文件直接存储，大文件存路径）")
    sql_file_path = Column(String(500), comment="SQL文件存储路径（大文件时使用）")
    ...
    rollback_sql = Column(Text, comment="回滚SQL（小文件直接存储）")
    rollback_file_path = Column(String(500), comment="回滚SQL文件存储路径（大文件时使用）")
    rollback_generated = Column(Boolean, default=False, comment="是否已生成回滚SQL")
    file_storage_type = Column(String(20), default="database", comment="存储类型：database/local/s3/oss")
