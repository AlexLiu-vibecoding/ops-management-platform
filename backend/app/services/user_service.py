...
class UserService(BaseService[User]):
    ...
    def update_last_login(self, user_id: int, ip: str) -> bool:
        """
        更新最后登录信息
    ...
    def can_manage_user(self, operator: User, target_user: User) -> bool:
        ...
        """
        # 超级管理员可以管理所有人
        if self.is_super_admin(operator):
            return True
        
        # 管理员只能管理普通用户
        if self.is_admin(operator) and target_user.role == UserRole.READONLY:
            return True
